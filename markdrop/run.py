...
setup_keys(
    key="google"
)  # or setup_keys(key = 'openai'), if using models = ['openai'] in the generate_descriptions function
...
models = [
    "gemini"
]  # Replace it with the desired models from ['qwen', 'gemini', 'openai', 'llama-vision', 'molmo', 'pixtral'] only

generate_descriptions(
    input_path=input_path, output_dir=output_dir, prompt=prompt, llm_client=models
)
